...
def local_adjust_options(options, args):
    ...
    f.write(urlopen('https://pypi.python.org/packages/source/s/setuptools/setuptools-2.0.2.tar.gz').read())  # NOQA
    f.close()
    print('setuptools downloaded')

    print('download pip...')
    f = open(path_join(tmp_dir, 'pip-latest.tar.gz'), 'w')
    f.write(urlopen('https://pypi.python.org/packages/source/p/pip/pip-1.5.tar.gz').read())  # NOQA
    f.close()
